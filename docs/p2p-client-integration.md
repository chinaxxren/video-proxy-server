...
P2P-enabled native builds fail if the resulting library does not export the
callback and directory registration APIs, complete verification, and remove.
...
## Managed Client Adapters

Swift, Kotlin, and ArkTS use the directory Provider API so managed objects do
not need to survive synchronous callbacks on arbitrary Rust worker threads.
The Host writes authorized pieces under an absolute, app-private directory as:

```text
<piece-directory>/0.piece
<piece-directory>/1.piece
...
```

Core constructs these numeric filenames itself, limits each read to 8 MiB, and
verifies the bytes against the Manifest before serving them. This API does not
download pieces, discover peers, or grant authorization. The Host must keep the
directory available until it removes the source.
...
cache.removeP2PSource(sourceId) // authorization revocation
```

The Swift methods are available when `MEDIA_PROXY_CACHE_ENABLE_P2P` is defined
for both Swift and C:
...
HarmonyOS uses a decimal string for the opaque ID to avoid JavaScript number
precision loss:
...
The default Android and HarmonyOS libraries retain these managed methods but
registration fails clearly because P2P is not compiled in. The iOS methods are
omitted unless the compile-time P2P definition is present.
